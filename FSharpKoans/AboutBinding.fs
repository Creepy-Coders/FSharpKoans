...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "Mudiwa" // replace with a string
        let z = true // replace with anything else!
    ...
    let ``09 A wildcard pattern will match anything`` () =
        ...
        let _ = "Jade" // replace with a string
        let _ = false // replace with anything else!
        ()
    ...
    let ``11 Constant patterns fail if the sides don't match exactly`` () =
        ...
        (fun () ->
            let "FILL ME IN" = "FILL__ME_IN_TOO"
            ()
